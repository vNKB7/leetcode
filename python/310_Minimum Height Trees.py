...
'''
#TLE
# 分别把每一个节点作为初始节点，广度优先遍历。直接从edges中搜索边，速度太慢
...
'''
#TLE 改为邻接链表的表示，但是没有用visit数组记录访问情况，还是太慢
class Solution(object):
    def findMinHeightTrees(self, n, edges):
        """
        :type n: int
...
class Solution(object):
    def findMinHeightTrees(self, n, edges):
        ...
        end = [i for i in range(n) if degree[i] == 1]
        remain = set(i for i in range(n) if degree[i] > 1)
